...
do {
 ...
 precioVenta= calcularMontoVenta(idProducto,cantidadVenta);
   ...
   do{
    cin >> idVendedor;
    vendedorEncontrado=buscarVendedor(idVendedor);
    if(!vendedorEncontrado){
        cout<<"El ID no esta dado de alta, ingese otro ID de vendedor: "<<endl;
    }
...
void GestorDeVenta::agregarCliente(){ // ESTE GUARDA Y CREA EN TEORIA, LLAMA A CREAR DENTRO DE GUARDAR

    Cliente nuevoCliente = ingresarNuevoCliente();
...
Cliente GestorDeVenta:: ingresarNuevoCliente(){
...
void GestorDeVenta::listarVendedores(){
 ...
 void GestorDeVenta:: mostrarVendedores(Vendedor &reg){
        ...
        cout<<"ID vendedor: "<<reg.getIdVendedor();
